...
import {generateChoiceName} from "../utils.mjs";
...
export default class JournalDialoguePageSheet extends JournalPageSheet {
  ...
  get #nextNodeChoices() {
    const pages = this.document.parent.pages.filter(p => (p.type === CONSTANTS.JOURNAL_ENTRY_PAGE_TYPE) && (p.id !== this.document.id));
  ...
  activateListeners($html) {
    ...
    const inputsOnAccordion = html.querySelectorAll(".choice-header input:not([disabled])");   
    inputsOnAccordion.forEach(input => {
  ...
  async _onCreateChoice(element) {
    const choices = this.document.system.choices;
    const newChoiceId = foundry.utils.randomID();
