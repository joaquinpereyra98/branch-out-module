 ...
 * @property {direction} [direction]     The direction in which the accordion expands. One of "vertical" or "horizontal".
 */
...
export default class Accordion {
  ...
  bind(root) {
    ...
    const {headingSelector, contentSelector, startCollapsed, startExpanded, direction} = this.#config;
    ...
    for (const heading of root.querySelectorAll(headingSelector)) {
      ...
      const isCollapsed = firstBind
        ? forceExpand
          ? false
          : forceCollapse || (this.#collapsed.length > 0)
        : this.#collapsed[collapsedIndex];

      if (firstBind) this.#collapsed.push(isCollapsed);

      heading.classList.toggle("collapsed", isCollapsed);
      gsap.set(content, { 
        [direction === Accordion.DIRECTION.VERTICAL ? "height" : "width"]: isCollapsed ? 0 : "auto", 
        overflow: "hidden" 
      });
  ...
  _onExpandSection(heading, content) {
    ...
    gsap.to(content, {[this.#config.direction === Accordion.DIRECTION.VERTICAL ? "height" : "width"]: "auto", duration: 0.5, overflow: "hidden"});
  ...
  _onCollapseSection(heading, content) {
    ...
    gsap.to(content, {[this.#config.direction === Accordion.DIRECTION.VERTICAL ? "height" : "width"]: 0, duration: 0.5, overflow: "hidden"});
    const icon = heading.querySelector(".accordion-icon");
    if (icon)
      gsap.to(icon, {
        rotation: "-=180",
